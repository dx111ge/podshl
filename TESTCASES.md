...
| L2 | NVIDIA + Wayland | Needs `WEBKIT_DISABLE_DMABUF_RENDERER=1`. `mise run client` sets it; the `.deb` does not yet, so `INSTALL.md` tells the person to — a launcher that sets it is the fix | manual |
