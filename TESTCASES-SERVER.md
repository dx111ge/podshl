...
| **SV121** | **A project nobody has touched in a year** | **Is asked about daily, not every fifteen minutes. Only a *failing* source backed off; one that answered and had not changed was asked again in fifteen minutes for ever, which `record_ingest`'s own docstring already said was wrong — ten thousand projects at that interval is eleven requests a second, nearly all `304`. The interval is a twenty-fourth of the quiet time, floored at fifteen minutes so the period right after a change stays prompt and capped at a day so anchor re-verification keeps `stale` at fourteen days a promise; a change resets it to the floor however long the quiet before it was. Proved red against the flat interval** | auto |
