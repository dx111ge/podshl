...
fn a_refused_or_unprivileged_request_changes_nothing() {
    ...
    let run = |args: &[&str]| std::process::Command::new(exe).args(args).status()
        .expect("the helper did not start").code();
    assert_eq!(run(&[]), Some(exit::REFUSED));
    assert_eq!(run(&["run_powershell", "command=calc"]), Some(exit::REFUSED));
    assert_eq!(run(&["set_machine_env", "name=PATH", "value=x"]), Some(exit::REFUSED));

    #[cfg(windows)]
    {
        let name = "PODSHL_ELEVATE_TEST";
        let before = elevated::read("set_machine_env",
        ...
        if is_elevated() {
            ...
            assert_eq!(got, Some(exit::DENIED), "an unprivileged helper did not report denial");
            let after = elevated::read("set_machine_env",
                &[("name".to_string(), name.to_string())].into_iter().collect());
            assert_eq!(before, after, "a denied request changed the machine");
        }
        assert_eq!(run(&["restart_service", "service=PodshlNoSuchService"]).map(|c| c == exit::DENIED || c == exit::NOT_FOUND),
                   Some(true));
    }
    #[cfg(not(windows))]
    assert_eq!(run(&["restart_service", "service=cups"]), Some(exit::NOT_WINDOWS));
}
...
fn is_elevated() -> bool {
    ...
    unsafe {
        ...
        let mut len = 0u32;
        let ok = GetTokenInformation(token, TokenElevation, &mut e as *mut _ as *mut _,
