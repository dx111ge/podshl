...
Not built yet: resolving `fixed_in` from the upstream issue itself (a merged
pull request's commit, then the first release tag that contains it). Today it
is only what the publisher states.
