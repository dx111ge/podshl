...
| **I7** | **Which languages exist, and how many the window loads** | **`languages.json` names every code and the name that language calls itself, and is held to the directory: a code with no file is a language that fails when somebody picks it, a file with no entry is one nobody can reach, and `I1` sees neither because it only compares the tables it found. The window takes its list from there and declares none of its own — it used to carry `const LANGUAGES = […]` **and fetch every table at startup to use one**: 42 KB each, 169 KB at four, growing with a number this product means to grow while the benefit stays at one language. English is loaded because every sentence falls back to it, the chosen language beside it, the rest only when somebody picks them. A language the client does not have is now said in English rather than silently swapped for it** | auto |
...
| **OM4** | **A fresh install on Omarchy uses the desktop's agent without being asked to** | **Fixed more than once, and back every time, because every check ran on a machine where the agent had already been chosen by hand.** OM1 to OM3 held how the agent is called and when it is *offered*; nothing held that a client with no `~/.config/podshl/llm.json` *uses* it, and the walks that said "the chip reads `claude`" ran with that file in place. The first install from the package on 2026-09-16 said **No own model** on a desktop whose default agent is `claude`. `llm::load` now resolves a missing file to the agent `omarchy::offer()` vouches for; a saved choice wins, including a saved choice of none, and nothing is written, so the default is recomputed on every start. Where the prompt goes is still said on every read panel, from `is_cloud()`. Proved red by resolving without the agent. **The unit test is the floor, not the proof**: the proof is a start with an empty `~/.config/podshl` on the Omarchy machine** | auto |
| **OM5** | **Asking the agent leaves no transcript behind** | **Every headless call left its whole session under `~/.claude/projects/-tmp-podshl-agent-…`: 29 directories of 76 KB on the first desktop, each holding the prompt, and so every reading the person had allowed, on disk for good in another program's history. `--no-session-persistence` (measured with Claude Code 2.1.273) stops the transcript and still leaves an empty directory with an empty `memory/`; the client removes those with `remove_dir`, which refuses anything that is not empty, so a directory the agent did write into stays as it is. The real call is `omarchy::tests::asking_the_agent_leaves_no_session_behind`, ignored because it spends a completion: run on the Omarchy machine with `cargo test -- --ignored`, green with the fix and red without it — it named the directory it found** | auto |
| **CL1** | **The client is built for an operator, not for loopback** | **A client built without `PODSHL_BUILD_SERVER_URL` and `PODSHL_BUILD_LOG_KEY` is not a broken client, it is a *plausible* one: it starts, draws its window, and quietly cannot verify the published directory — so every published project falls through to the model, which then asks what the project is, having never been told. `option_env!` is read at compile time and cargo does not rebuild when only an environment variable changed, so this is one `cargo build` away at any moment. `scripts/build/build_client.sh` builds it and then checks the binary rather than trusting the build; this asks the binary itself** | auto |
...
| **CL4** | **The whole published path answers, from the project** | **Search, card, diagnosis, with no model anywhere in it. The answer names the fix the project published and says it rested on something a person typed. **Since 2026-09-16 it also walks the case the operating-system reading exists for**, which was wrong twice over: the probe asked `os_fact` for a fact it does not answer and read nothing on every machine, and once that was fixed the right answer was still unreachable on an x86_64 desktop because it says nothing about the processor (`SV112`). Somebody on Linux holding the Windows archive is now told so by name; somebody holding the right archive gets the class's own fallback rather than a rule about another machine, and somebody else's repository on the same forge reaches none of it. **The archive matrix lacked the commonest case**: an Intel Linux desktop holding the ARM build, which engram itself calls the most common reason. Walked on Omarchy on 2026-09-16 it answered "find out which archive you have" to somebody who had just said; engram now publishes `wrong-build-on-an-intel-machine`, and this case asks for it** | auto |
...
| **C3** | **Consent panel focus** | **The *refusing* button holds focus, so a stray Return can never grant. Two checks, and the pair is the point: the source is held to it here, which is weak on its own because a panel could take focus back; and `scripts/walk/drive_window.mjs` checks the live `document.activeElement` on **every** consent panel it meets, which is strong and needs a desktop. Every walk on 2026-09-12 reported "refusing button not focused on: none"** | auto |
...
A change outlives its reason: the bug it works around gets fixed, an update
overwrites it, and a year later nobody knows why it is there. So the client
writes down every change before making it, and looks again (`repair.rs`).

| # | Case | Expected | Cover |
|---|---|---|---|
| **RR1** | **A change is on record before it is made** | **Before the file is touched, `repairs.json` holds the action, its parameters, the file as resolved, the backup, whether it can be undone, who proposed it, what the publisher said about the software (package, upstream issue, the version that fixes it) and the version the package manager reports. The case reads the file from inside the version lookup and finds it unchanged. Every field a decision reads is written by code; a model's `note` is kept and read by nothing** | auto |
| **RR2** | **A record that cannot be written** | **Stops the change before its backup. A list that exists and cannot be read is not an empty list: it is left as it was and the change is refused, because writing a new list would erase every earlier record** | auto |
| **RR3** | **Looking again** | **When the window opens, every applied change is compared with the machine: a package at another version is flagged, the version upstream named as the fix is flagged as "may no longer be needed — or the fix did not hold", a build from a repository's head cannot be compared and says so, a change something rewrote is flagged. Nothing is undone and neither the file nor its backup is touched. A person who keeps a change is not asked again until the next update; an undo takes the record off the list and keeps it** | auto |
| **RR4** | **Which version is newer** | **As pacman orders it — epoch, then version, then release where both have one; `1.0rc` before `1.0`, `1.10` after `1.9`, `1.5.a` after `1.5` and `1.5b` before it. Every expectation is what `vercmp` from `archlinux:base` printed, including the one that looked wrong. A `-git` package, or a version naming a commit, is not ordered at all** | auto |
| **RR5** | **What a publisher says about the software** | **Is checked as text on both sides before anything uses it: a package name that would need escaping, an issue that is not `https`, a version with anything but version characters, or a key nobody defined, is refused — by the client when a remedy arrives and when a change is made, and by the operator's gate when a solution proposes one** | auto |

| **RR6** | **A file changed by another tool** | **`repairs begin` keeps a copy before the change and `done` records the file's digest after it; a later edit or a missing file is flagged, `keep` hides it until something new happens, `restore` puts the copy back and marks the record undone. `add` records a change already made, with no copy. A path that does not exist, a `by` longer than 80 characters or with control characters, and a note over 2000 characters are refused** | auto |
| **RR7** | **A package built or pinned locally** | **Keeps the installed version and the version the package manager offered then. When the offer moves on — to a newer version, or to any other version than the one recorded, because a local `9999` build never looks older — the record says the package is held back and names both versions** | auto |
| **RR8** | **A local copy in front of a component** | **Keeps the digest of the copy and of what it overrides (a path's digest, or a package's version). When the original changes underneath it, the record is flagged; an unchanged original is not** | auto |
| **RR9** | **Watching the upstream issue** | **Off unless switched on per record, and only for a GitHub issue or pull request link. A watched record is asked at most once a day and never after the answer is final; `--offline` and an unwatched record ask nothing. A merged pull request is followed to the first published release after the merge that contains the merge commit (drafts and pre-releases skipped, a tag GitHub cannot compare skipped); that tag is compared with the installed version like `fixed_in`. A closed issue and a merged pull request without a release are flagged. Walked live against `cli/cli` pull 14289: `v2.99.0`** | auto |
| **RR10** | **Looking again after updates, or daily** | **`repairs install-hook` says each step before taking it and sets up only `repairs review --notify`: an Omarchy post-update hook, a systemd user timer elsewhere on Linux, a scheduled task without elevation on Windows, a LaunchAgent on macOS. A path that could break out of the line is refused, `remove-hook` writes nothing, and an unknown command or option is named. The review exits 0 with nothing to look at, 3 with something, 1 on error. Walked 2026-09-18 on Windows 11 (task created, run, the toast confirmed in the notification database, removed twice) on a real Omarchy desktop (the Omarchy hook chosen over systemd, a real `omarchy-update` ran it, the notification reached the desktop over D-Bus, the update ended 0, `remove-hook` left the other hooks alone), on a hosted macOS runner (the plist written, `plutil`-clean, loaded, listed, started and removed; `osascript` invoked, though no banner can be seen without a session), and on a plain Linux with a live systemd user session (`docker/systemd-walk.Dockerfile`: the units accepted, the timer enabled and scheduled, exit 3 recorded by systemd as `Result=success`, no notification daemon needed, `remove-hook` leaving nothing). Every system's hook has now been run on that system** | auto |
| **RR11** | **One record, said in the singular** | **The notification has a sentence of its own for a single record, in every language the client speaks, and it does not open with the count. The plural keeps the count. Found on a real Omarchy desktop, where the only sentence there was read "1 recorded local fixes want another look" — and "1 aufgezeichnete lokale Korrekturen", "1 correcciones locales registradas", "1 corrections locales enregistrées"** | auto |
| **RR12** | **The notification is PODSHL's own** | **The toast is raised under `de.podshl.client` — the identifier the installer gives the program — and never under the identity of the program that happens to raise it. The name is claimed with a `DisplayName` in the current user's own hive, through `reg.exe` rather than the registry-writing calls that belong to the elevate helper (`L5`). Before this the toast arrived named "Windows PowerShell", grouped under PowerShell in the Action Center and silenced by PowerShell's notification setting; that is why the first Windows walk did not find the notification it had just sent** | auto |
| **RR16** | **Removing a record** | **`repairs forget ID` removes what a record said and the copy it kept, and leaves a stub: the id, when the change was recorded, its kind, and when it was forgotten — so a ledger with a hole in it does not read like a ledger that never had the entry. It refuses without administrator or root rights, and refuses again unless a person types the record's id at a real terminal; a pipe is not a person and no option skips either gate. A forgotten record is not raised by a review and cannot be forgotten twice. The removal is written to the log, which is then the only place that still says what the record held. Walked as root in a pty: removed on the id, refused on anything else and on a bare return** | auto |
| **RR14** | **What it did, written down** | **Every `repairs` command that changes something writes a line: what was recorded, what the review concluded and with which flags — by their own names, not a translated sentence — what was restored, and what the hook set up or took away. The one lookup that leaves the machine, the GitHub check for a watched issue, is logged with what it answered; `--offline` asks and logs nothing. The log is anonymised like a report, and `PODSHL_LOG=0` writes nothing at all. It wrote nothing before, which mattered most where the hook runs unattended behind `|| true`** | auto |
| **RR15** | **Uninstalling takes back what was registered** | **The daily scheduled task and the notification identity live outside the install folder and so survive it. Both are removed by the uninstaller, without the person having run `remove-hook` first, and neither waits on the "delete application data" box; an update keeps both. Walked on Windows 11: task and key present before, gone after** | auto |
| **RR13** | **The command that writes a file says which** | **`restore` is the only one of `done`, `keep` and `restore` that changes anything on disk, and it said nothing at all — exit 0 and a silently rewritten file. It now names the file it put back, with the path shortened the way every other path is** | auto |
| **RR17** | **One record, from either program** | **`podshl-repairs` is the record without the window, not a second record. A fix begun by one program is finished by the other, flagged by the first, listed identically by both and restored by the second — each step taken by the program that did not take the last one** | auto |
| **RR18** | **`podshl-repairs` carries no window** | **Checked on the built file, because that is what somebody installs: on Linux it asks the loader for no WebKit, GTK, GDK, JavaScriptCore or libsoup; on Windows its header says console program, so a shell waits for it and reads its exit code without `Start-Process -Wait`. And it introduces itself as `podshl-repairs`: its help began with `podshl-client repairs <command>`, the other program** | auto |
| **RR19** | **The hook `podshl-repairs` writes is one it can run** | **`install-hook` writes the running program followed by `repairs review --notify`, the client's spelling. From `podshl-repairs` that line was refused as an unknown command — daily, behind the hook's `|| true`, unseen. `podshl-repairs` accepts `repairs` in front, and the case takes the printed hook line apart and runs it** | auto |
| **RR20** | **The notification names the program that raised it** | **It said "Open PODSHL, or run: podshl-client repairs review" whoever raised it, so somebody with only `podshl-repairs` was sent to a window and a program they do not have. From `podshl-repairs` it now says `podshl-repairs review`. On Omarchy it goes through `omarchy-notification-send` as critical — Omarchy shows a normal notice for 8 seconds, in the middle of an update, and a critical one until it is dismissed — and a click opens a floating terminal with the review; the path in that click is quoted, and one that cannot be is not passed. The next notice replaces the last, so updates nobody looked after leave one notice rather than a pile: walked on Omarchy, twice in a row left one (the same id), and once dismissed the next came up new. Walked on the Omarchy desktop on 2026-09-19: the `Notify` call carried the sentence and the click, read off D-Bus; the notice stayed on screen, and clicking it opened the floating terminal with the review — seen by the person at that desktop** | auto |
| **RR21** | **What a coding agent writes is recorded without anyone asking** | **Through the agent's own hooks: before a file write a copy is kept, after it the record is finished, by the agent's name. One record per file per session, its copy from before the session and its digest brought up to date on every later edit; a file the agent creates is recorded without a copy. Not recorded: anything in a git working tree, temporary files, the agent's own directory, the record itself. Input it cannot read is an error on stderr and exit 0 — the agent is never stopped. Walked on Omarchy on 2026-09-19 with Claude Code 2.1.278 as the default agent: a real `claude -p` edit to `~/.config` was begun and finished and then restored; the same edit inside a fresh git repository left no record** | auto |
| **RR22** | **Installing the agent hook changes nothing else** | **`install-agent-hook` takes Omarchy's default agent unless one is named, refuses an agent whose hook format has not been measured (only `claude` so far), and writes two entries into the agent's settings: everything else stays, installing twice leaves one, and `remove-agent-hook` takes out only its own. The change to the settings file is itself recorded, with a copy** | auto |

## What the window shows

| # | Case | Expected | Cover |
|---|---|---|---|
| **AN2** | **A path in the person's own profile** | **Is shown as `%LOCALAPPDATA%\…`, `%APPDATA%\…`, `%USERPROFILE%\…` or `~/…`, never with the account name in it — the window is photographed and pasted into issues, and this project's own published screenshots carried the developer's account name. Only a whole folder is replaced (`C:\Users\ann` is not a prefix of `C:\Users\anna`), the longest prefix wins, and Windows compares without case** | auto |

## Administrator actions

Three examples, agreed on 2026-09-17 as a start to be sharpened (`L5`).

| # | Case | Expected | Cover |
|---|---|---|---|
| **EV1** | **An action that needs administrator rights** | **Is one of three, with its parameters in their exact form, or it is refused before Windows is asked anything. Services Windows needs to run, update or defend itself and variables every program reads to find itself (`PATH`, `SystemRoot`, `TEMP`, …) are refused by name, whatever the case** | auto |
| **EV2** | **The helper, started as the program it is** | **Checks the request again with the same rules and refuses an unknown action, a repeated or missing parameter, and a protected name. Started without administrator rights it reports that and the machine reads the same afterwards. Walked by hand with the prompt answered on 2026-09-17 (`elevate::tests::walk_the_prompt`, ignored unless asked for): a machine variable set, read back by the client, undone through the recorded undo, and absent from the registry afterwards** | auto |
| **EV3** | **What counts as done** | **The client reads the service or the variable before and after, without privilege; the change is recorded as made only when the second reading shows it, whatever the helper answered. Its undo is the first reading — the previous start type, the previous value, or removing a variable that did not exist — and a start type the helper will not set is never offered as one. A restart has nothing to undo** | auto |
...
| **W21** | **A maintainer who has published and proved control** | **Can reach their own project from any page. The dashboard existed and no page linked it: every navigation read *Publish · Register* and stopped, so `/dashboard` was reachable only by knowing the URL. Found by the maintainer of this project looking for their own reports. It goes after *Register* because that is the order the work happens in** | auto |
| **W22** | **Which code this operator is running** | **Said by the operator, in the footer of every page and in `GET /`. The mirror publishes the commit of every project it serves so that anybody can check it against the source, and the operator itself said nothing — the only way to know what `sdota.de` ran was to ask the maintainer, which is the shape of claim this project exists to replace. `git describe --tags --always` at deploy time, baked into the image as a build argument so the string travels with the code it describes. The whole chain is checked — Dockerfile, compose, `update.sh`, the route, the pages — and a build nobody told says nothing rather than claiming a version. The 503 page for an unconfigured deployment is exempt: it fetches nothing at all** | auto |
| **W19** | **The files the builder on `/publish/build` writes** | **Taken by the mirror. The writer is a block with no page in it, run in node on real drafts — readings from the vocabulary, a question with choices, conditions with operators and "one of", a version `1.10` YAML would read as a number, a glossary, an escalation, and the example the page offers — and what it writes is sent to `/validate`, which runs the ingest checks (`SV108`). Every scalar is written as a JSON string, so YAML's typing never changes what a maintainer typed. **A third draft since 2026-09-16: a project that has stopped.** `successor` is a key ingest accepts and the form had no field for, so a maintainer following *Saying a project is finished* had to hand-edit what the builder wrote — it was never lost, because unknown keys are kept verbatim and named in the UI, but kept is not the same as writable. Checked both ways: a deprecated project gets the line, an active one never does** | auto |
...
| **L3** | **Windows branch** | **Builds, runs and passes the whole suite on Windows, and every path has been walked through the real window: published (including a log excerpt read from a file), vendor across all three skills in four languages, the model path against Ollama, the hand-off to a person, free text, and the settings panel. Bundled since 2026-09-13: `scripts/build/build_windows_installer.ps1` builds a per-user NSIS setup with the operator's address and log key compiled in. That setup was installed silently (no administrator; nothing under HKLM), the installed program started from an unrelated directory with no `VS_*` or `PODSHL_*` in its environment walked the published path to a verified answer, and the silent uninstall removed the program, both shortcuts and the uninstall entry. `manual` because it needs a Windows desktop and is walked per release — **walked for 0.1.3 on 2026-09-16**: installed silently from `var/release/0.1.3/`, HKCU only and nothing under HKLM, started from `C:\Windows\Temp` with every `VS_*` and `PODSHL_*` removed and no `../var` within reach, and it still answered `has_key: true` and verified sdota.de's signed index at `tree_size: 4`. State landed in `%APPDATA%\podshl` and not beside the program (`L10`). Uninstalled silently: registry entry, program folder and shortcuts all gone. **Walked again for 0.1.5 on 2026-09-18**, this time with the repair record on it: installed silently; the installed program prints into a real console (`AttachConsole`), though as a window program the shell returns to the prompt before the output arrives; `repairs install-hook` created the daily task without elevation and `schtasks /Run` raised a notification confirmed in `wpndatabase.db`; and the uninstaller removed the program, the shortcut, the task and the notification identity without `remove-hook` having been run first. **Not signed** — SmartScreen names no publisher; a certificate is a decision, not work** | manual |
| **UI1** | **The flow decided, and the flow walked** | **Three layers outside this table, in `client-rs/uitest/`, run by `scripts/ci/ci.sh` rather than by `run_testcases.py` — so they are listed here as `manual` and are not claimed as `auto`, which this document's own drift check would otherwise report as missing. The *decisions* (`ui/flow.js`) are tested with no page and no network in milliseconds; the *shipped page* is served to a headless browser under the application's own CSP with `invoke` bridged to the real binary; the *real window* is driven over WebView2's DevTools protocol, which is Windows only and is the one a person still runs by hand. Every case in the first two is a defect this project shipped** | manual |
| L4 | macOS branch | **The binary is verified, the window is not.** Since 2026-09-18 a hosted runner runs the client on macOS — it starts, the cases that need no counterparty pass, and the repair record works there end to end (`macos-walk.yml`). What stays `open` is the part a runner has no way to reach: the window itself, and the `.dmg` a person installs. Both need a Mac in front of somebody | open |
| **L5** | **Windows elevation** | **Done by a separate program, never in process. Three example actions need administrator rights (`restart_service`, `set_service_start`, `set_machine_env`, marked `elevated` in `actions.json`), and only `podshl-elevate` performs them: the client asks Windows to start it with the `runas` verb, so Windows shows its own prompt, and the request is the program's command line and its answer the exit code — no file a program running as administrator would read from, or write to, in a folder the person's other programs can write. The client's source is held to containing none of the calls that write a service or the machine's registry, and the helper's to reading no file, writing none, starting nothing and opening no connection. The examples are to be sharpened; the rule is not** | auto |
