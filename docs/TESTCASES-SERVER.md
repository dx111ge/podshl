...
| **SV122** | **A maintainer asks where the reports about their project are** | **They are on their dashboard, and only theirs. A report about a project on a forge carries the repository URL as its subject — the client sends `pick.base` for a repository anchor — while the dashboard looked for the host in its own path, `github.com`, which no report has ever carried. So from the day repository anchors existed (`0018`) every such report was stored correctly, counted correctly, and shown to nobody: engram had two clusters and a page reading zero. Found by the maintainer asking. Matching the bare host would be the opposite mistake and a worse one, so both directions are checked — two repositories are claimed on one forge, one is reported about, and the neighbour sees none of it (`SV21`)** | auto |
| **SV120** | **A solution that says nothing about a fact another solution branches on** | **Is reachable under every value of that fact. It used to live only inside the branches some other rule happened to create, so a reading outside those fell off the tree and took the answer with it — published, mirrored, signed and unreachable. engram found it on a real desktop: `wrong-archive-for-this-system` names the operating system and the download and nothing about the processor, another rule names `os.arch: aarch64`, and an ordinary x86_64 Linux machine holding the Windows archive was told to go and find out which archive it had. A last child matching every value the author did not name carries exactly those solutions; a named value still wins, a class where every rule constrains the switch grows no such branch and still says nothing outside it, and a tree with the catch-all in front of a named value is refused** | auto |
...
| **SV116** | **A confusable owner is held, and the forge is not the claimant** | **`hold_reason` examined the labels of the host, which for a repository is the forge's — so the check asked whether *GitHub* resembles somebody's mark, a question whose answers are "no, for everyone" and "hold every project on GitHub", and neither is about the claimant. Impersonation on a forge is in the **owner**: `dx111geo/engram` is a letter out and the host is identical either way. The owner and name are what is examined now, mixed scripts there are named as such, and an ordinary repository on the same forge is not held. Known and accepted: a mark's owner is recognised by host, so the same vendor's own repository on a forge is held for carrying its own mark — held is not blocked, and recognising an owner on a forge would mean deciding whose name is whose** | auto |
| **SV117** | **A repository may publish the endpoint a person can visit** | **A repository anchor proves two locations at once — control is shown by writing into the repository, which is as much a fact about `github.com/owner/name/` as about the raw prefix the bytes are read from. `check_endpoint` compared against the fetch prefix alone, so a maintainer's own issue tracker was refused while a URL under a content delivery host nobody visits would have been accepted. Found in production: the first real project enrolled and its source was refused on exactly that. Somebody else's repository on the same forge is still refused, and a domain keeps the narrower rule — its fetch prefix, not its whole host** | auto |
...
| **SV118** | **A notice filed behind a backlog is still reachable** | **`pending` was oldest first with a hard limit and no offset. The order is right, because a queue is worked from the front — but the hundred-and-first undecided notice was not on a later page, it was on no page at all, so an operator holding a backlog could never see, or act on, anything filed after the queue filled up. Found on 2026-09-15 when three cases that file a notice and then look for it went red in a development database with 292 pending: they were the operator, and the operator could not see it either. `/notices` takes an offset and says `total`; the case files behind its own backlog, checks two pages do not overlap, and finds the last one** | auto |
| **SV119** | **A freshly stood-up operator can serve its index** | **`GET /index` runs in a read-only transaction, enforced by Postgres rather than by a flag — and `sth.current` issued a head when it found none, which is a write. So the first request to an operator that had never signed anything answered 500 with `cannot execute INSERT in a read-only transaction`. Production never met it, because a head has existed there since its first crawl; anybody standing up their own operator meets it immediately, which is the whole of the self-hosting story. Found by the staging instance on the day it was built, which is the first thing staging was built to do. The migration runner issues the first head, where writing is allowed, and `current` now reports honestly that there is none** | auto |
...
Designed in [INGEST-REDESIGN.md](INGEST-REDESIGN.md), agreed on 2026-09-16 and built on 2026-09-17. These
replace `SV121`, the ramp that asked every source daily at most, whether anybody used it or not.
...
| SV126 | A cold source is asked about | Its files are checked before anything is served; unchanged is served under the existing log entry, changed is stored and logged first; the source becomes hot | auto |
| SV127 | A cold source whose files cannot be checked | Is not served. The answer says the project's current files cannot be checked right now, not that the project publishes nothing | auto |
| SV128 | A solution withdrawn while its project was cold | Is never served: the check before serving removes it first | auto |
| SV129 | Every anchor, used or not | Is checked weekly; `stale` after 14 days and `unknown` after 90 still arrive on time for a project nobody uses. Found while building it: `sweep.apply_grades` had no caller, so an anchor whose challenge disappeared recorded `failing_since` and stayed `live` for ever | auto |
| SV130 | Many cold sources asked about at once | On-demand checks run in a bounded pool, one per source, with back-off after a failure; a request waits about five seconds and is then told the project is being loaded | auto |
| SV131 | A project with reports about it goes cold | Its stored files, its trees, its clusters and the reports in them are still there when it has cooled and when it is used again. The design named `cluster_partition`, which `0015` had dropped along with `link`; `cluster.source_id` is what hangs off a project today | auto |
| SV132 | What a query leaves behind | At most the day its source was last used, written at most once a day and outside the query's read-only transaction; no time, count or requester | auto |
| SV133 | A solution whose content does not match its manifest digest | Is refused like any invalid file, and the maintainer's dashboard names the file and the digest. With a digest for every file, a check of an unchanged project is the manifest's `304` and nothing else; a digest spelled so it could never match is refused when the manifest is parsed | auto |
